...
namespace shadowfactory.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        /// <summary>
        /// SIMPLE LOGIN - PURE ADO.NET (100% WORKING)
        /// </summary>
        [HttpPost("login")]
        public async Task<IActionResult> Login([FromBody] UserLoginRequest request)
        {
            try
            {
                ...
                // ⭐⭐⭐ SIMPLEST POSSIBLE QUERY - ONLY COLUMNS THAT DEFINITELY EXIST IN DATABASE ⭐⭐⭐
                // Based on your database schema, use this:
                var sql = @"
                    SELECT 
                        Id, 
                        Email, 
                ...
                {
                    var factorySql = @"
                        SELECT Id, FactoryName, FactoryNameEn, IndustryType, Location, Verified, Status 
                        FROM Factories 
                        WHERE Id = @FactoryId";
                    ...
                    if (await factoryReader.ReadAsync())
                    {
                        factoryDto = new FactoryDto
                        {
                            ...
                            Verified = factoryReader.GetBoolean(5),
